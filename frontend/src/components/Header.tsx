...
import BrochureDownloadForm from './contact/BrochureDownloadForm'; // Import the new component

const Header = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isScrolled, setIsScrolled] = useState(false);
  const [showBrochureForm, setShowBrochureForm] = useState(false); // New state for the form
  ...
  // Close the mobile menu and open the form
    ...
    <>
      ...
      >
        <div className="container mx-auto px-4 flex justify-between items-center">
          {/* Logo Section */}
          <Link to="/" className="flex items-center gap-3 h-20">
            <img
              src="/assets/tr-logo-1.webp"
              alt="TR Baking Equipments Logo"
              height={90}
              width={90}
              onError={(e) => { e.currentTarget.src = 'https://placehold.co/80x80/EAEAEA/3C3C3C?text=TR+Logo'; }}
            />
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {navLinks.map((link) => (
              <NavLink
                ...
                style={({ isActive }) => ({
                  color: isActive ? '#593D2B' : '#3C3C3C',
                  borderBottomColor: isActive ? '#CB6843' : 'transparent',
                })}
              >
                {link.name}
              </NavLink>
              ...
              <div className="flex flex-col space-y-2">
                {navLinks.map((link) => (
                  <NavLink
                    ...
                    style={({ isActive }) => ({
                      color: '#3C3C3C',
                      backgroundColor: isActive ? '#F5F0E6' : 'white',
                      borderLeftColor: isActive ? '#CB6843' : 'transparent',
                    })}
                ...
                <button
                  ...
                  className="mt-2 py-3 text-center font-medium tracking-wider"
                  style={{
                    backgroundColor: '#CB6843',
                    color: 'white',
                    borderRadius: '4px',
      ...
      {showBrochureForm && (
        <BrochureDownloadForm 
          ...
          brochureUrl="/assets/TR-Baking-Equipments-Brochure.pdf" // Update with the correct path to your brochure file
        />
      )}
    </>
  );
